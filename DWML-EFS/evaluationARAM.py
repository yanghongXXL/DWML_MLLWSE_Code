...
def classify(X_train, y_train, X_test, y_test, classifier, mtrs):
    ...
    # 检查预测结果是否为稀疏矩阵并相应地进行转换
    if hasattr(prediction, 'toarray'):
    	prediction = prediction.toarray()

    for m in mtrs:
        if m == 'hamming':
            results[m] = metrics.hamming_loss(y_test, prediction)

        if m == 'ranking':
            results[m] = metrics.label_ranking_loss(y_test, prediction)

        if m == 'coverage':
            results[m] = metrics.coverage_error(y_test, prediction)
        ...
        if m == 'jaccardscore':
            results[m] = metrics.jaccard_score(y_test, prediction, average='weighted')

        if m == 'microfscore':
            results[m] = microfscore(y_test, prediction)

        if m == 'zeroone':
            results[m] = metrics.zero_one_loss(y_test, prediction)

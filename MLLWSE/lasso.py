...
def lasso(X,Y,alpha,beta,enta,maxIter,miniLossMargin):
    ...
    W_s = np.dot(np.linalg.inv(XTX + enta * np.eye(n_features)), XTY).astype(np.float64)
    W_s_1 = W_s
    # Calculate the similarity distance
    Y_array = np.asarray(Y.todense()) if sp.issparse(Y) else np.asarray(Y)
    H = pairwise_distances(np.transpose(Y_array), metric="cosine")
    iter = 1
    oldloss = 0
    ...
    while iter<=maxIter:
        W_s_k = W_s + np.dot((bk_1 - 1) / bk ,(W_s - W_s_1)).astype(np.float64)
